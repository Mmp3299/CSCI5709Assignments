...
import { AppBar, Box, Toolbar, Container, MenuItem, Typography, Drawer, IconButton, Avatar, Link as MuiLink } from '@mui/material';
import { Link as RouterLink } from 'react-router-dom';
import { useTheme, useMediaQuery } from '@mui/material';
import { appBarStyle, toolbarStyle, boxStyle, logoStyle, menuStyle, profileBoxStyle, drawerBoxStyle, drawerContentStyle } from '../styles/Navbar.module.js';
          ...
          <Toolbar variant="regular" sx={toolbarStyle(theme)}>
            <Box sx={boxStyle}>
              <MuiLink component={RouterLink} to="/" color="text.secondary">
                <img src="/appLogo-circular.png" style={logoStyle} alt="logo of sitemark" />
              </MuiLink>
              <Box sx={menuStyle}>
                {extendedMenuItems.map((item) => (
                  <MenuItem key={item.text} sx={{ py: '6px', px: '12px' }}>
                    <MuiLink component={RouterLink} to={item.path} color="text.primary">
                ...
                <Box sx={drawerContentStyle(isSmallScreen)}>
                  {menuItems.concat(profileItems).map((item) => (
                    <MenuItem key={item.text}>
                      <MuiLink component={RouterLink} to={item.path}>{item.text}</MuiLink>
                    </MenuItem>
                  ))}
                </Box>
              </Drawer>
